...
async function ordersLoad() {
  ...
  setText('order-stat-processing', data.processing || 0);
  setText('order-stat-failed', data.failed || 0);
  setText('order-stat-refunded', data.refunded || 0);

  // 更新底部统计
  const totalEl = document.getElementById('orders-total-info');
  if (totalEl) {
  ...
  ordersRenderTable();
  ordersRenderPagination();
  ordersUpdateBatchActions();
}

function ordersUpdateSummary(data) {
  const items = data.items || [];
  const total = data.total || 0;
  const totalAmount = items.reduce((sum, o) => sum + (parseFloat(o.amount) || 0), 0);
  const successAmount = items.filter(o => o.status === 2).reduce((sum, o) => sum + (parseFloat(o.amount) || 0), 0);
  const failedAmount = items.filter(o => o.status === 3).reduce((sum, o) => sum + (parseFloat(o.amount) || 0), 0);

  const setText = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
  setText('order-summary-total', total);
  setText('order-summary-amount', `¥${totalAmount.toLocaleString('zh-CN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`);
  setText('order-summary-success-amount', `¥${successAmount.toLocaleString('zh-CN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`);
  setText('order-summary-failed-amount', `¥${failedAmount.toLocaleString('zh-CN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`);
}

function ordersFilterStatus(el) {
  const status = el.getAttribute('data-status');
  _orders.statusFilter = status ? parseInt(status) : null;
  _orders.page = 1;

  // 更新状态标签
  document.querySelectorAll('.status-tab').forEach(e => e.classList.remove('active'));
  el.classList.add('active');
...
function ordersReset() {
  ...
  document.querySelectorAll('.status-tab').forEach((e, i) => {
    e.classList.toggle('active', i === 0);
  });
...
function ordersRenderTable() {
  ...
  if (!_orders.filtered.length) {
    tbody.innerHTML = `<tr><td colspan="16" style="text-align:center;padding:40px;color:var(--text-3);">暂无数据</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = _orders.filtered.map(o => {
    ...
    const callbackTime = o.callback_at ? o.callback_at.replace(' ', ' ').slice(0, 16) : '—';
    const account = o.remark ? o.remark.replace(/^充值账号:\s*/, '') : '—';
    const elapsed = o.elapsed || '—';
    ...
    return `<tr data-order-id="${o.id}">
      ...
      <td>
        ${actionButtons}
        <button class="btn-link" onclick="orderDetailOpen(${o.id})" style="margin-left:5px;">详情</button>
      </td>
    </tr>`;
  }).join('');
}

function ordersRenderPagination() {
  const pagination = document.getElementById('orders-pagination');
  if (!pagination) return;

  const total = document.getElementById('orders-total-info').textContent.match(/共 (\d+) 条记录/);
  const totalCount = total ? parseInt(total[1]) : 0;
  const totalPages = Math.ceil(totalCount / _orders.limit) || 1;
  const currentPage = _orders.page;

  let html = '';
  
  // 上一页
  html += `<button class="page-btn" ${currentPage <= 1 ? 'disabled' : ''} onclick="_orders.page=${currentPage-1};ordersLoad()">‹</button>`;
  
  // 页码按钮
  if (totalPages <= 7) {
    for (let i = 1; i <= totalPages; i++) {
      html += `<button class="page-btn${i === currentPage ? ' active' : ''}" onclick="_orders.page=${i};ordersLoad()">${i}</button>`;
    }
  } else {
    html += `<button class="page-btn${1 === currentPage ? ' active' : ''}" onclick="_orders.page=1;ordersLoad()">1</button>`;
    ...
    html += `<button class="page-btn${totalPages === currentPage ? ' active' : ''}" onclick="_orders.page=${totalPages};ordersLoad()">${totalPages}</button>`;
  }
  
  // 下一页
  html += `<button class="page-btn" ${currentPage >= totalPages ? 'disabled' : ''} onclick="_orders.page=${currentPage+1};ordersLoad()">›</button>`;
  
  pagination.innerHTML = html;
}

function ordersUpdateBatchActions() {
  const checkedCount = document.querySelectorAll('.order-checkbox:checked').length;
  const batchBar = document.getElementById('order-batch-actions');
...
function ordersSelectAll(checkbox) {
...
function ordersBatchUpdate(newStatus) {
  const checkedBoxes = document.querySelectorAll('.order-checkbox:checked');
  ...
  const statusNames = { 0: '待处理', 1: '处理中', 2: '成功', 3: '失败', 4: '已退款' };
  if (!confirm(`确定要将选中的 ${checkedBoxes.length} 条订单状态改为"${statusNames[newStatus]}"吗？`)) {
    return;
  }
  ...
  Promise.all(orderIds.map(orderId => 
    ...
    showToast(`已成功更新 ${orderIds.length} 条订单`, 'success');
