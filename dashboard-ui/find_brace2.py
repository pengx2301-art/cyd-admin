...
while i < len(content):
    ...
    # 处理字符串
    if not in_double and not in_backtick and not in_line_comment and not in_block_comment:
        if c == "'":
            in_single = not in_single
    if not in_single and not in_backtick and not in_line_comment and not in_block_comment:
        if c == '"':
    ...
    if not in_single and not in_double and not in_backtick and not in_line_comment and not in_block_comment:
        if c == '{':
            ...
            if depth < 0:
                ...
                print(f'Line {line_num}: Extra closing brace at position {i}')
                print(f'Context: {content[max(0,i-50):min(len(content),i+50)]}')
                break
